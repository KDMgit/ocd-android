...
OCD.urls.geonode.signup = OCD.urls.geonode.host + '/mobile/signup';
...
OCD.gps.getCurrenthId = navigator.geolocation.getCurrentPosition(
		OCD.gps.onSuccess, OCD.gps.onError, OCD.gps.options);
OCD.gps.watchId = navigator.geolocation.watchPosition(OCD.gps.onSuccess,
		OCD.gps.onError, OCD.gps.options);
...
$.ajaxSetup({
	beforeSend : function(xhr, settings) {
		...
		if (!csrfSafeMethod(settings.type)) {
			xhr.setRequestHeader("X-CSRFToken", OCD.ajax.cookie['csrftoken']);
		}
	}
});

OCD.ajax.getHeaders = function() {
	cookie = OCD.ajax.cookie;
